...
const sketch = ({ width, height }) => {
  ...
  const systems = ['#f13401', '#11804b', '#0769ce', '#f1d93c'].map((color) => ({
    u: [
      Random.rangeFloor(width * 0.2, width * 0.8),
      Random.rangeFloor(height * 0.2, height * 0.8),
    ],
    v: [
      Random.rangeFloor(width * 0.2, width * 0.8),
      Random.rangeFloor(height * 0.2, height * 0.8),
...
function system({ u, v, w, context, width, height, playhead, color }) {
  ...
  drawGeometry(
    context,
    { u: nextU, v: nextV, w: nextW },
    { width, height, margin: config.margin },
    color
  );
}
...
function drawGeometry(context, { u, v, w }, { width, height, margin }, color) {
  ...
  const triangles = [
    // [u, v, w],
    [[margin, margin], u, [width - margin, margin]],
    [u, [width - margin, margin], [width - margin, height - margin]],
    [u, [margin, height - margin], [width - margin, height - margin]],
    [u, [margin, height - margin], [margin, margin]],
  ];
